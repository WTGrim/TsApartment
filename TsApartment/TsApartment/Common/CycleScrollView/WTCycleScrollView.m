...
- (void)initPageControl{
    ...
    if (self.onlyShowText || self.imageUrlsArray.count == 0 || self.imageUrlsArray.count == 1) return;
...
//        self.pageControl.frame = CGRectMake((self.frame.size.width - width) * 0.5, self.frame.size.height - 15, width, 8);
        CGSize size = [_pageControl sizeForNumberOfPages:self.imagePathsArray.count];
        if (self.imagePathsArray.count == 0) {
            _pageControl.bounds = CGRectZero;
        }else{
            _pageControl.bounds = CGRectMake(0, 0, SCREEN_WIDTH, size.height);
        }
        _pageControl.center = CGPointMake(self.center.x, CGRectGetMaxY(self.bounds) - 5);
    }
