...
- (void)setupTitleView{
    ...
    if (!self.titleStyle.isScrollTitle) {//如果不是滑动的，那么平分宽度
        ...
        if (self.titleStyle.sliderWidthFitTitle) {
            ...
            float allTitlesWidth = self.titleStyle.titleMargin;
            for (int i = 0; i<self.titleWidths.count; i++) {
                allTitlesWidth = allTitlesWidth + [self.titleWidths[i] floatValue] + self.titleStyle.titleMargin;
            }
        ...
        for (TitleView *titleView in self.titleViews) {
            ...
            titleViewX = lastTitleMaxX + addedMargin/2;
            
            lastTitleMaxX += (titleViewW + addedMargin + self.titleStyle.titleMargin);
...
- (UIView *)bottomLine{
    ...
    if (!_bottomLine) {
        ...
        bottomLine.backgroundColor = [UIColor colorWithRed:200/255.0 green:199/255.0 blue:204/255.0 alpha:1];
